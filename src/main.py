...
async def place_buy_order(request: Request, payload: Dict[str, Any] = Body(...), _: str = Depends(verify_api_key)) -> JSONResponse:
...
async def restart_server() -> JSONResponse:
    ...
    """
    try:
        subprocess.run("/usr/bin/sudo /usr/bin/systemctl restart uma-scalper", shell=True, check=True)
