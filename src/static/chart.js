window.addEventListener("DOMContentLoaded", () => {
	...
	function setupChart(containerId, symbol, buttonIds, settings) {
		...
		chartContainer.addEventListener('mousedown', (e) => {
			if (!stopLine && !targetLine && !buyLine) return;
			const rect = chartContainer.getBoundingClientRect();
			const y = e.clientY - rect.top;
			const priceScale = candleSeries.priceScale();
			const priceRange = priceScale.getVisibleRange();
			const height = rect.height - 60; // approximate chart area
			
			const priceAtY = (y) => {
				const range = priceRange.upper - priceRange.lower;
				return priceRange.upper - (y / height) * range;
			};
			
			const lines = [
				{ line: buyLine, name: 'buy' },
				{ line: stopLine, name: 'stop' },
				{ line: targetLine, name: 'target' }
			];
			
			for (const { line, name } of lines) {
				if (!line) continue;
				const lineY = priceScale.priceToCoordinate(line.options.price);
				if (Math.abs(y - lineY) < 10) {
					draggingLine = { line, name };
		...
		chartContainer.addEventListener('mousemove', (e) => {
			...
			if (priceRange) {
				const range = priceRange.upper - priceRange.lower;
				newPrice = priceRange.upper - (y / height) * range;
			}
			
			if (newPrice !== undefined) {
				newPrice = Math.round(newPrice * 100) / 100;
				draggingLine.line.applyOptions({ price: newPrice });
				ohlcDiv.textContent = `${draggingLine.name.toUpperCase()}: ${newPrice}`;
			}
		});
