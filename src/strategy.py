...
class Strategy:
    ...
    def __init__(self, user_settings, ltp_of_underlying):
        ...
        self.tokens_for_all_trading_symbols.update(self.sym.get_tokens(user_settings["atm"]))
    ...
    def find_trading_symbol_by_atm(self, ce_or_pe, quotes): 
        ...
        logging.info(f"premium {self.user_settings['premium']} to be check against quotes {quotes} for closeness ")
        quotes = {self.tokens_for_all_trading_symbols[k]: v for k, v in quotes.items()}
        symbol_with_closest_premium = self.sym.find_closest_premium(quotes=quotes, premium=self.user_settings["premium"], contains=ce_or_pe)
