...
async def trading_session_stop(app: Any) -> None:
    ...
    if getattr(_logic_state, "ws", None) and hasattr(
...
async def start_logic() -> dict[str, Any]:
    ...
    return {
        ...
        "started_at": (
            _logic_state.started_at.isoformat() if _logic_state.started_at else None
        ),
    }
...
async def pause_logic(
    reason: str = "manual", duration_seconds: int = 60
) -> dict[str, Any]:
    if _logic_state.is_running():
        await trading_session_stop(None)
